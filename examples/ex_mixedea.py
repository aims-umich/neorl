...
for item in ['mixed', 'grid', 'float/int', 'float/grid', 'int/grid', 'float', 'int']:
    ...
    sa=SA(mode='min', bounds=bounds, fit=Vessel, chain_size=20, chi=0.2, Tmax=10000, threshold = 1, lmbda=0.05,
          move_func=my_move, reinforce_best='soft', cooling='equilibrium', ncores=4, seed=1)

    x_sa, y_sa, sa_hist=sa.evolute(ngen=ngen, verbose=0)
    assert Vessel(x_sa) == y_sa
    x_sa, y_sa, sa_hist=sa.evolute(ngen=ngen, x0=x0[0:4], verbose=0)
    assert Vessel(x_sa) == y_sa
