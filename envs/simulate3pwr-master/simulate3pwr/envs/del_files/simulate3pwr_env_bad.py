...
class SIMULcolEnv3(gym.Env):
  ...
  def __init__ (self, casename='method', log_dir='./master_log/'):#, max_episode = 10, run_number = 12): # --- '12' because of my default initial file
    ...
    self.numlocs = 55 # --- size of the board to make test on : 4 9 16 25 40
    self.max_episode = 10 # --- tuning parameter
    self.run_number = 12
    ...
    self.default_state = list(range(0,56))
    self.VECTOR = []
    self.core = []
    self.librairies = {}
  ...
  def _num_nonzero(self,arr):
        """
        Get the number of non-zero element in the provided array
  ...
  def step(self, action): # --- < int > : position
    ...
    self.file = self.casename + "_"+ str(self.counter) + '_%f.inp'%(self.fileindex)
    self.state_file = self.casename + "_"+ str(self.counter) + '_%f.out'%(self.fileindex)
  ...
  def clean(self):
    ...
    #os.system('find . -type f -name "*_gym.out" -exec rm {} \;')
    #os.system('find . -type f -name "*_gym.inp" -exec rm {} \;')
    #os.system('find . -type f -name "*_gym.res" -exec rm {} \;')
    #os.system('find . -type f -name "*_gym.txt" -exec rm {} \;')
    os.system('rm -rf {file}.inp {file}.out /{res}/{file}.res /{res}/{file}.sum'.format(file=self.fileheuristic,res =self.res))
    # --- maybe later
    #os.system("rm -rf run%d.out run%d.inp /%s/run%d_12.*"%(self.iter - 2,self.iter - 2,self.res,self.iter - 2))
  ...
  def monitor(self,reward):
      ...
      with open (self.log_dir+self.casename+'_out.csv','a') as fin:
          fin.write(str(np.round(self.fileindex)) + ', ' + str(reward) + ', '+ str(self.max_peaking) + ', '+ str(self.max_deltah) + ', ' + str(self.max_boron) + 
      ...
      print('CaseID:', self.fileheuristic)
      print('Reward:', reward)
  ...
  def write_input(self, flag = True): # --- write new input file
    ...
    if flag: # --- multicycle depletion
        ...
        for cycle_number in range(12,17):#int(self.title.split()[1]) + 1,int(self.title.split()[1]) + 6): # --- write en external file for each depletion cycle
            ...
            f3 = open('loadcycle_%f_%d_gym.txt'%(self.fileindex,cycle_number),'w')
            ...
            f2 = open(self.file,'a')
            f2.writelines("\n")
            s = "'" + dirname + '/loadcycle_%f_%d_gym.txt'%(self.fileindex,cycle_number) + "'"
...
if __name__ == '__main__':
    ...
    for env in list(gym.envs.registry.env_specs):
      if 'simulate3pwr' in env:
          print("Remove {} from registry".format(env))
          del gym.envs.registry.env_specs[env]
          
    env=gym.make('simulate3pwr:simulate3pwr-v0')